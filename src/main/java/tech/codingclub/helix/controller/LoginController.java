...
@Controller
@RequestMapping("/login") //this will hear to slash login
public class LoginController extends BaseController {
    ...
    LoginResponse handleLogin(@RequestBody Member member, HttpServletRequest request, HttpServletResponse response) {
        ...
        if(x!=null && x.size()>0)
        {
            //condition is met..email and password are correct
            Member memberTemp=x.get(0);
            memberTemp.role="cm";
            ControllerUtils.setUserSession(request,memberTemp);//cookie provide expiry date
